...
def align(element):
	...
	commands = """
	makeblastdb -dbtype nucl -in {sample}_phylo_contigs.fasta
	blastn -query {sample}_phylo_contigs.fasta -db conus_phylogenetic_targets_sliced_v2.fa -outfmt 6 -word_size 11 -out {sample}_6.1.blastoutput -evalue 1e-10
	blastn -query conus_phylogenetic_targets_sliced_v2.fa -db {sample}_phylo_contigs.fasta -outfmt 6 -word_size 11 -out {sample}_6.1.recipblastoutput -evalue 1e-10
	python contig_filter.v3.py {sample}.exon.cov {sample}_phylo_contigs.fasta {sample}_6.1.recipblastoutput {sample}_6.1
	""".format(**variables)
