...
# if the target is already in keepdict, only replace it if the coverage is higher. Else, add it to keepdict.
		...
		if ID == 'MAP44':
			savethis = [coverage, ID, blastdict['imperialis_192289_96|noex|first_design|no_exon_boundaries']]
		elif ID == 'MAP33':
			savethis2 = [coverage, ID, blastdict['index7_122247|noex|first_design|no_exon_boundaries']]
		

####################################################### now only deal with the 'best' reference per target #######

keepdict['imperialis_192289_96|noex|first_design|no_exon_boundaries'] = savethis ## needed to do this because MAP44 is imperialis, and another contig was being blasted in other species..
keepdict['index7_122247|noex|first_design|no_exon_boundaries'] = savethis2
